...
class Batch2048EnvSimulator():
    ...
    def move(boards: np.ndarray, action: int | list | tuple | np.ndarray, reward=False, use_ray=False):
        ...
        if idx.size:
            if reward:
                Batch2048EnvSimulator._cal_reward(boards, idx, lut_left_reward, rewards)
            Batch2048EnvSimulator._apply_lut_inplace(boards, idx, lut_left)

        # DOWN -> RIGHT while transposed, RIGHT
        idx = np.nonzero((action == 1) | (action == 3))[0]
        if idx.size:
            if reward:
                Batch2048EnvSimulator._cal_reward(boards, idx, lut_right_reward, rewards)
            Batch2048EnvSimulator._apply_lut_inplace(boards, idx, lut_right)
